  ...
  $(document).ready(function() {
	  $("#searchFilter").keyup(function () {
		  ...
		  for (i = 0; i < tr.length; i++) {
			  ...
			  if(fn.toUpperCase().indexOf(filter) > -1 && $(tr[i]).find("#fieldLabelHidden").val() != 'deleted'){
				  tr[i].style.display = "";
			  }
			  else {
	  ...
	  $("#fieldLabel").keyup(function(){
		  var suggestedName = $("#fieldLabel").val().replace(/\s+/g, '_').toLowerCase();;
		  if(suggestedName[suggestedName.length -1] == '-'){
			  suggestedName = suggestedName.slice(0, -1);
		  }
		  $("#fieldName").val(suggestedName)
	  })
	  ...
	  $("#fieldName,#fieldLabel").focusout(function(){
	            ...
	            if (ui.item) {
	        		...
	        		if(!exists){	        			
	        	    	...
	        	    	$("#fieldTypeIdHidden").val(ui.item.fieldTypeId);
	        	  		...
	        	  		console.log(ui.item.regex);
	        	  		$("#regexPat").val(ui.item.regex);
	        	  		addFieldRow();
	        	  		form[0].reset();
	        	   }
	        		else{
	...
	function addFieldRow(){
    	...
    	$("#tblFields tbody" ).prepend( 
                ...
                +"<input type='hidden' id='fModelOrList' name='fieldsList["+count+"].modelOrList' value="+($("#radioModelOrList:checked").length > 0 ? $("#radioModelOrList:checked").val() : null) +">"
                +"<input type='hidden' id='fModelName' name='fieldsList["+count+"].modelName' value='"+ ($("#selectModel option:selected").length ? $("#selectModel option:selected").val() : null) +"'>"
                ...
                +"<input type='hidden' id='regexPatHidden' name='fieldsList["+count+"].regex' value='"+$("#regexPat").val()+"'>"
                +"</spring:bind>"
                +"</td>"
    ...
    function editField()
    {
    	...
    	$("#"+$("#edittrid").val()).find("#regexPatHidden").val($("#regexPat").val());
    	$("#isedit").val("");
    	/*New add - end*/
    }
...
*/      }
    ...
    $( "#fieldTypeId" ).change(function() {
    	...
    	if($("#fieldTypeId option:selected").text() == "text"){
    		$("#trPattern").css('display','table-row');
    		$("#regexPat").val(null);
    	}
    	else{
    		$("#trPattern").css('display','none');
    	}
    	
    	if($("#fieldTypeId option:selected").text() == "select" || $("#fieldTypeId option:selected").text() == "checkbox" || $("#fieldTypeId option:selected").text() == "radio" ){
    		console.log("changed");
...
/*	  var obj = $(obj).closest('tr');
  ...
  function copyFieldValuesFromTrToModalForm(obj){
	  ...
	  $("#fieldTypeId").val(obj.find("#fTypeId").val());
	  $("#fieldTypeName").val(obj.find("#fTypeName").val());
	  if(obj.find("#fTypeName").val() == "text"){
		  $("#trPattern").css('display','table-row');
		  console.log(obj.find("#regexPatHidden").val());
		  $("#regexPat").val(obj.find("#regexPatHidden").val());
		  console.log(obj.find("#regexPat").val());
	  }
	  if(obj.find("#fTypeName").val() == "select" || obj.find("#fTypeName").val() == "checkbox" || obj.find("#fTypeName").val() == "radio"){
		  $("#trModelOrList").css('display','table-row');
		  if(obj.find("#fModelOrList").val() == "model"){
			  $('#radioModelOrList[value="' + obj.find("#fModelOrList").val() + '"]').prop('checked',true);
			  $("#trModel").css("display","table-row");
